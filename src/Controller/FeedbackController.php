...
final class FeedbackController extends AbstractController
{
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                ...
                if ($analysisResult) {
                    ...
                    $feedback->setSentimentLabel($analysisResult['sentiment_label']);
                ...
                // Update doctor's average score via Flask API
    ...
    public function delete(Request $request, Feedback $feedback, EntityManagerInterface $entityManager, MedecinSyncService $syncService, LoggerInterface $logger): Response
    {
        // Check if patient is the owner of this feedback
        $patientId = $request->getSession()->get('patient_id');
        if (!$patientId || $feedback->getPatient()->getId() !== $patientId) {
        ...
        if ($this->isCsrfTokenValid('delete'.$feedback->getId(), $request->getPayload()->getString('_token'))) {
            try {
                ...
                // Recalculate doctor's average score after deletion via Flask API
                if ($doctor) {
                    $syncService->syncAiScore($doctor);
                }
                
                $this->addFlash('success', 'Feedback deleted successfully!');
                $logger->info('Feedback deleted, doctor score synchronized via Flask API', [
                    'doctor_id' => $doctor ? $doctor->getId() : null
                ]);
            } catch (\Exception $e) {
