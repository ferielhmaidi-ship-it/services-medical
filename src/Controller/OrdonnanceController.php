...
class OrdonnanceController extends AbstractController
{
    ...
    public function add(Request $request, EntityManagerInterface $em): Response
    {
        ...
        $qb = $em->getRepository(Appointment::class)
            ...
            ->andWhere('a.doctorId = :doctorId')
            ->setParameter('doctorId', $medecin->getId())
            ->orderBy('a.date', 'DESC')
            ->addOrderBy('a.startTime', 'DESC');
        if ($selectedPatient instanceof Patient) {
            $qb->andWhere('a.patientId = :patientId')
                ->setParameter('patientId', $selectedPatient->getId());
        }
    ...
    public function mod(Request $request, EntityManagerInterface $em, int $id): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($currentAppointment instanceof Appointment) {
                return $this->redirectToRoute('rapports_ordonnances', [
                    'idpatient' => $currentAppointment->getPatientId(),
                    'idmedecin' => $medecin->getId(),
    ...
    {
        if ($appointment->getDoctorId() !== $medecin->getId()) {
    ...
    {
        if ($patient instanceof Patient && $appointment->getPatientId() !== $patient->getId()) {
    ...
    {
        $patient = $em->getRepository(Patient::class)->find($appointment->getPatientId());
