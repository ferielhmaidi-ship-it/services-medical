...
class HomeController extends AbstractController
{
    ...
    {
        $user = $this->getUser();
        $this->logger->info('submitAppointment hit. User: ' . ($user ? $user->getUserIdentifier() : 'null'));

        if (!$user || !($user instanceof \App\Entity\Patient)) {
            $this->logger->warning('Booking denied: User not logged in as Patient');
        ...
        $this->logger->info('Doctor lookup: ' . ($doctor ? $doctor->getId() : 'not found') . ' for ID ' . $doctorId);
        ...
        try {
            ...
            $this->logger->error('Email confirmation failed in HomeController: ' . $e->getMessage());
        }
