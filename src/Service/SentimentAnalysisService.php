...
class SentimentAnalysisService
{
    ...
    /**
     * Calculate doctor average sentiment score using all feedbacks
     * 
     * @param array $feedbacks Array of ['comment' => string, 'rating' => int]
     * @return array{
     *     average_sentiment_score: float|null,
     *     average_rating: float|null,
     *     total_feedbacks: int
     * }|null
     */
    public function getDoctorSentimentScore(array $feedbacks): ?array
    {
        try {
            $response = $this->httpClient->request('POST', $this->flaskUrl . '/doctor-sentiment-score', [
                'json' => ['feedbacks' => $feedbacks],
                'timeout' => 30
            ]);
            ...
            $this->logger->error('Flask API doctor score error', [
                'status' => $response->getStatusCode()
            ]);
            return null;
            ...
            $this->logger->error('Doctor sentiment calculation failed: ' . $e->getMessage());
            return null;
        }
    }
}
