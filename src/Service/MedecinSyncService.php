...
class MedecinSyncService
{
    ...
    public function syncAiScore(Medecin $doctor): bool
    {
        ...
        if ($result && isset($result['average_sentiment_score'])) {
            $avgScore = (float)$result['average_sentiment_score'];
            $doctor->setAiAverageScore($avgScore);
            
            // Calculate label on Symfony side (mirroring Flask logic)
            $label = 'neutral';
            if ($avgScore >= 4) {
                $label = 'very_positive';
            } elseif ($avgScore >= 3) {
            ...
            $doctor->setAiSentimentLabel($label);

            $doctor->setAiScoreUpdatedAt(new \DateTimeImmutable());
            $this->em->flush();
            
            $this->logger->info(' Doctor AI score and label synced', [
