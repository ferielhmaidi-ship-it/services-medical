<?php

declare(strict_types=1);

namespace DoctrineMigrations;

use Doctrine\DBAL\Schema\Schema;
use Doctrine\Migrations\AbstractMigration;

/**
 * Auto-generated Migration: Please modify to your needs!
 */
final class Version20260225132713 extends AbstractMigration
{
    public function getDescription(): string
    {
        return '';
    }

    public function up(Schema $schema): void
    {
        // this up() migration is auto-generated, please modify it to your needs
        $this->addSql('ALTER TABLE article ADD summary LONGTEXT DEFAULT NULL');
        $this->addSql('ALTER TABLE feedback DROP sentiment_label');
        $this->addSql('ALTER TABLE medecins DROP ai_sentiment_label');
    }

    public function down(Schema $schema): void
    {
        // this down() migration is auto-generated, please modify it to your needs
        $this->addSql('ALTER TABLE article DROP summary');
        $this->addSql('ALTER TABLE feedback ADD sentiment_label VARCHAR(20) DEFAULT NULL');
        $this->addSql('ALTER TABLE medecins ADD ai_sentiment_label VARCHAR(20) DEFAULT NULL');
    }
}
